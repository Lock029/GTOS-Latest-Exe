# GTOS-Latest-Exe
GTOS LATEST EXE LEAKED THIS IS NOT MY PROBLEM mal#0009 

Updates : 1.32 (Current Version)

Added Rift Cape, `NOT FULLY DONE YET`!
All Latest GTOS Custom Items!

How To Setup??
1. Add a folder Called save and in it u add folders such as bluemailbox, bulletinboard, donationboxes, gemdb, guildrewards, guilds, mailbox, mannequin, players, registerdIP, rift, savevault, storageboxlvl1, storageboxlvl2, storageboxlvl3, temp, worldbans, worlds For The Ipban Part ( Add ipbans Folder and in it u add aid, gid, ip, ip_id, mac, rid, sid, vid) Done!
2. Open Config.json Change port to yr choice
3. Edit News.json If U Wish
4. Set Yr Admin Level As 2 (Developer Role)
5. Enjoy!

Note : This Leakage Is Caused Due To Mal Arrogant And Dont Even Help The Community When They Need Help Besides That Asks Them To Ping Me For No Reason, mal#0009 Device Ban Me From GTOS With A Reason Steal Exe Which Is True Cause I Knew He Would Do Something Similar Like This, Btw Dont Buy Exe From Sebia Since Its Leaked Here! Enjoy!!

ANYTHING DM Time#1337
